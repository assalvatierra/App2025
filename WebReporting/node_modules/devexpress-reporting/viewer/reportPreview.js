...
import { ClipboardSeparator, EditablePreviewEnabled, HandlerUri, MessageHandler, PollingDelay, PreloadedPagesOffset } from './settings';
...
export class ReportPreview extends ReportHolder {
    ...
    constructor(handlerUri, previewRequestWrapper, previewHandlersHelper, callbacks, rtl = false, enableKeyboardSupport, exportSettings, element, breadcrumb) {
        ...
        this.getSelectedContent = (punctuationMark = '', bricks) => {
            ...
            let result = '';
            const clipboardSeparator = ClipboardSeparator();
            sortedActiveBricks.forEach((row, index) => {
                for (let c = firstUsedColumn; c <= lastUsedColumn; c++) {
                    const cellText = row[c] ? row[c] + punctuationMark : '';
                    result += c == lastUsedColumn ? cellText : extendWithSpaces(currentPage.brickColumnWidthArray[c], cellText);
                    if (c != lastUsedColumn && clipboardSeparator) {
                        result += clipboardSeparator;
                    }
                }
