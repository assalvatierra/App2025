...
export function getDBSchemaCallback({ requestWrapper, connection, tables, getTables, getViews }) {
    ...
    requestWrapper.getDbSchema({ connection, tables, getTables, getViews })
        ...
        .fail(data => {
        ShowMessage(formatUnicorn(getLocalization('Schema loading failed. {0}', 'DxDesignerStringId.Error_SchemaLoadingFailed'), getErrorMessage(data)), 'error', 5000);
        deferred.reject();
    });
...
export function getDBStoredProceduresCallback(requestWrapper, connection) {
    ...
    requestWrapper.getDbStoredProcedures(connection)
        ...
        .fail(data => {
        ShowMessage(formatUnicorn(getLocalization('Stored procedures loading failed. {0}', 'DxDesignerStringId.Error_SchemaLoadingFailed'), getErrorMessage(data)), 'error', 5000);
        deferred.reject();
    });
    return deferred.promise();
}
