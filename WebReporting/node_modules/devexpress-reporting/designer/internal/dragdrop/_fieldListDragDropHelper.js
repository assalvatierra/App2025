...
import { makeFieldDisplayName } from '../_displayNameProvider';
export class FieldListDragDropHelper {
    constructor(_dataBindingMode, _size, _performBindings = true) {
        this._dataBindingMode = _dataBindingMode;
        this._size = _size;
        this._performBindings = _performBindings;
    ...
    _setControlTextAlignment(cell, specifics) {
        const numericSpecifics = ['Integer', 'Float'];
        if (numericSpecifics.some(numericType => numericType === specifics)) {
            cell.textAlignment('TopRight');
        }
    }
    _createTable(parent, items) {
        ...
        items.forEach(item => {
            ...
            this._setControlTextAlignment(cell, item.data.specifics);
            if (!this._performBindings) {
                const formattedDisplayName = makeFieldDisplayName(item.data.displayName);
                cell.text(formattedDisplayName);
                return;
            }
...
export const listMemberControlsMap = {
    'List': {
        drop: (treeListItem, dropTargetControl, dataBindingMode, size, performBindings) => {
            const helper = new FieldListDragDropHelper(dataBindingMode, size, performBindings);
