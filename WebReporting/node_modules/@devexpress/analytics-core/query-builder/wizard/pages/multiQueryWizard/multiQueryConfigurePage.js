                ...
                if (node.name === 'tables' || node.name === 'views') {
                    this._itemsProvider().getItems(new PathRequest(node.name)).done(() => {
                        if (node.isList && node.children.peek().length > 0) {
                            DxDeferred.when(node.children.peek().map(item => this._getItemsPromise(new PathRequest(node.name + '.' + item.name))));
                        }
                    }).always(() => this._onChange());
                }
                else if (node.specifics === 'table' || node.specifics === 'view') {
                    this['_itemsProvider']().getItems(new PathRequest(node.specifics + '.' + node.name)).always(() => this._onChange());
                }
                else if (node.name === 'procedures') {
                    this['_itemsProvider']().getItems(new PathRequest(node.name)).always(() => this._onChange());
                }
            }
            else {
                this._onChange();
