...
export class PropertyGridKeyboardHelper extends AccessibilityKeyboardHelperBase {
    ...
    initialize() {
        Object.keys(this._complexEditorMap)
            .filter(headerId => !document.getElementById(headerId))
            .forEach(headerId => { delete this._complexEditorMap[headerId]; });
        for (const [key, value] of Object.entries(this._getComplexEditors())) {
            if (!this._complexEditorMap[key] || !this._complexEditorMap[key].isInitialized) {
                this._complexEditorMap[key] = value;
            }
        }
