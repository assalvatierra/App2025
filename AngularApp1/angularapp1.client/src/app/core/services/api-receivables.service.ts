...
export class ApiReceivablesService {
  ...
  getReceivable(id: number): Observable<Receivable> {
    return this.http.get<Receivable>(`${this.baseUrl}/api/Receivables/byId/${id}`).pipe(
      map((receivable: any) => this.mapReceivable(receivable)),
      catchError((error) => {
        console.error('Error fetching receivable:', error);
        return throwError(() => new Error(`Failed to fetch receivable with id ${id}: ${error.message || error.statusText}`));
      })
    );
  }
