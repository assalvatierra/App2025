...
function _uploadFile(filesHolder, deferred, options) {
    try {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            const isPlaintext = !options.readMode ? (format === 'html' || format === 'htm') : options.readMode === 'text';
            const isBase64 = !isPlaintext;
            fr.onload = () => {
                try {
                    deferred.resolve({
                        format: format,
                        content: _getFileContent(fr.result, options.readMode),
                        contentType: isBase64 ? 'base64' : 'plaintext'
                    });
                }
                catch (e) {
                    deferred.reject();
            ...
            isPlaintext ? fr.readAsText(file) : fr.readAsDataURL(file);
        }
    }
    catch (e) {
        deferred.reject();
